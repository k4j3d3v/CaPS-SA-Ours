...
void Suffix_Array<T_seq_, T_idx_>::sort_subarrays()
{
    ...
    const auto sort_subarr =
        [&](const idx_t i)
        {
            ...
            if(++solved_ % 1000 == 0) CAPS_SA_LOG(std::cerr << "\rSorted " << solved_ << " subarrays.");
        };
...
void Suffix_Array<T_seq_, T_idx_>::merge_sub_subarrays()
{
    ...
    const auto sort_part =
        [&](const idx_t j)
        {
            ...
            if(++solved_ % 1000 == 0) CAPS_SA_LOG(std::cerr << "\rMerged " << solved_ << " partitions.");
        };
...
template class CaPS_SA::Suffix_Array<uint16_t, uint32_t>;
template class CaPS_SA::Suffix_Array<uint16_t, uint64_t>;
